...
void aStar::pathFinder(POINT startPos, POINT endPos, POINT currentPos, list<POINT>& lMyWay)
{
	...
	for (int i = 0; i < _vOpenList.size(); i++)
	{
		if (_vOpenList[i]->getTopIdx().x == curIdxX && _vOpenList[i]->getTopIdx().y == curIdxY)
		{
	...
	// 앙재귀띠
	pathFinder(startPos, endPos, PointMake(tempTile->getTopIdx().x, tempTile->getTopIdx().y), lMyWay);
